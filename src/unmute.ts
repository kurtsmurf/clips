 ...
 */
export function unmute(context:AudioContext, allowBackgroundPlayback:boolean = false, forceIOSBehavior:boolean = false):{ dispose:{ ():void; }}
{
	//#region Helpers
	// Determine the page visibility api
	let pageVisibilityAPI:{ hidden:"hidden", visibilitychange:"visibilitychange" };
	if (document.hidden !== undefined) pageVisibilityAPI = { hidden:"hidden", visibilitychange:"visibilitychange" };
	else if ((<any>document).webkitHidden !== undefined) pageVisibilityAPI = { hidden: <any>"webkitHidden", visibilitychange: <any>"webkitvisibilitychange" };
	else if ((<any>document).mozHidden !== undefined) pageVisibilityAPI = { hidden: <any>"mozHidden", visibilitychange: <any>"mozvisibilitychange" };
	else if ((<any>document).msHidden !== undefined) pageVisibilityAPI = { hidden: <any>"msHidden", visibilitychange: <any>"msvisibilitychange" };

	// Helpers to add/remove a bunch of event listeners
	function addEventListeners(target:EventTarget, events:string[], handler:EventHandlerNonNull, capture:boolean, passive:boolean):void { for (let i:number = 0; i < events.length; ++i) target.addEventListener(events[i], handler, { capture: capture, passive: passive }); }
	function removeEventListeners(target:EventTarget, events:string[], handler:EventHandlerNonNull, capture:boolean, passive:boolean):void { for (let i:number = 0; i < events.length; ++i) target.removeEventListener(events[i], handler, <any>{ capture: capture, passive: passive }); }
	...
	const mediaPlaybackEvents:string[] = ["click", "contextmenu", "auxclick", "dblclick", "mousedown", "mouseup", "touchend", "keydown", "keyup"];
